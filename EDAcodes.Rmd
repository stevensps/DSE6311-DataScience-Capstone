...
# Clean variable names (optional but helpful)
brfss_subset <- brfss_subset %>% clean_names()

# Binary Variables → Encode as 0 = No, 1 = Yes
brfss_subset <- brfss_subset %>%
  mutate(
    hlthpl1 = ifelse(hlthpl1 == 1, 1, 0),
    persdoc3 = ifelse(persdoc3 == 1, 1, 0),
    medcost1 = ifelse(medcost1 == 1, 1, 0),
    exerany2 = ifelse(exerany2 == 1, 1, 0)
  )

# Ordinal Variables → Encode as ordered factors
brfss_subset <- brfss_subset %>%
  mutate(
    educa = factor(educa, levels = c(1,2,3,4,5,6), ordered = TRUE),
    incomg1 = factor(incomg1, levels = c(1,2,3,4,5,6,7,8), ordered = TRUE),
    checkup1 = factor(checkup1, levels = c(4,3,2,1), ordered = TRUE), # reverse order if necessary
    marital = factor(marital, levels = c(5,6,3,4,1,2), ordered = TRUE) # adjust order as appropriate
  )

# Nominal Variables → Convert to factor for one-hot encoding later if needed
brfss_subset <- brfss_subset %>%
  mutate(
    sex = factor(sex),
    race = factor(race),
...
brfss_subset <- dummy_cols(brfss_subset, select_columns = c("sex", "race", "primins1", "employ1"), remove_first_dummy = TRUE, remove_selected_columns = TRUE)
```
