...
set.seed(42)
train_indices <- sample(nrow(brfss_subset), size = floor(split_ratio * nrow(brfss_subset)))
train_df <- brfss_subset[train_indices, ]
test_df <- brfss_subset[-train_indices, ]
...
```{r}
set.seed(42)
train_indices <- sample(nrow(brfss_subset), size = floor(0.78 * nrow(brfss_subset)))
...
set.seed(123)
imputed_train <- mice(train_df, m = 1, method = "pmm", maxit = 5)
train_df <- complete(imputed_train)

set.seed(124)
imputed_test <- mice(test_df, m = 1, method = "pmm", maxit = 5)
...
cat("Missing values in test data:", sum(is.na(test_df)), "\n")

```

```{r}
train_df$mh_flag <- ifelse(train_df$addepev3 == 1 | train_df$decide == 1, 1, 0)
test_df$mh_flag <- ifelse(test_df$addepev3 == 1 | test_df$decide == 1, 1, 0)

```

```{r}
train_df$phys_active <- ifelse(train_df$exerany2 == "Yes" & train_df$pa150r4 == "Meets guidelines", 1, 0)
test_df$phys_active <- ifelse(test_df$exerany2 == "Yes" & test_df$pa150r4 == "Meets guidelines", 1, 0)

```

```{r}
to_binary <- function(x) ifelse(x == "Yes", 1, 0)

train_df$access_score <- rowSums(data.frame(
  to_binary(train_df$hlthpl1),
  to_binary(train_df$persdoc3),
  to_binary(train_df$medcost1)
))

test_df$access_score <- rowSums(data.frame(
  to_binary(test_df$hlthpl1),
  to_binary(test_df$persdoc3),
  to_binary(test_df$medcost1)
))

```

```{r}
train_df$income_group <- fct_collapse(train_df$incomg1,
  "Low" = c("Less than $10k", "$10k–$15k", "$15k–$20k"),
  "Mid" = c("$20k–$25k", "$25k–$35k", "$35k–$50k"),
  "High" = c("$50k–$75k", "Over $75k")
)

test_df$income_group <- fct_collapse(test_df$incomg1,
  "Low" = c("Less than $10k", "$10k–$15k", "$15k–$20k"),
  "Mid" = c("$20k–$25k", "$25k–$35k", "$35k–$50k"),
  "High" = c("$50k–$75k", "Over $75k")
)
...
```{r}
train_df$educa_group <- fct_collapse(train_df$educa,
  "HS_or_Less" = c("Never attended school", "Grades 1-8", "Grades 9-11", "High school grad"),
  "Some_College" = "Some college",
  "College_Grad" = "College grad"
)

test_df$educa_group <- fct_collapse(test_df$educa,
  "HS_or_Less" = c("Never attended school", "Grades 1-8", "Grades 9-11", "High school grad"),
  "Some_College" = "Some college",
  "College_Grad" = "College grad"
)
...
```{r}
str(train_encoded)
summary(train_encoded$ment14d_cat)
