...
print(significantchiResult) #Only list statistically significant results
```
...
```{r}
continuous_vars <- brfss_subset[, c("age80", "numadult", "state", "menthlth", "checkup1")]
ggpairs(continuous_vars)
```

#PCA biplot
```{r}
pca_result <- prcomp(brfss_numeric, scale. = TRUE) #SS 4.4 - Typically I believe PCA is better for higher dimensionality data, and we may want to consider excluding the response. Becuase of this, we may want to do away from the PCA biplot
biplot(pca_result, main = "PCA Biplot (Centered & Scaled Variables)")
```


# Pearson's Correlation to Measure Continuous Numeric
```{r}
...
#PCA biplot
```{r}
pca_result <- prcomp(brfss_numeric, scale. = TRUE) #SS 4.4 - Typically I believe PCA is better for higher dimensionality data, and we may want to consider excluding the response. Because of this, we may want to do away from the PCA biplot
biplot(pca_result, main = "PCA Biplot (Centered & Scaled Variables)")
```
