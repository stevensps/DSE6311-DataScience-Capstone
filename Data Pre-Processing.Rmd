...
```{r}
trainimputeFinal <- trainimputeFinal %>% #Categorize mental health days into three levels
  mutate(ment14d_cat = case_when(
    ment14d == 0 ~ "None", #Zero days
    ment14d <= 13 ~ "Occasional", #1-13 days
    ment14d >= 14 ~ "Frequent" #14+ days
  ))

testimputeFinal <- testimputeFinal %>% #Apply same categorization to test set
  mutate(ment14d_cat = case_when(
    ment14d == 0 ~ "None", #Zero days
    ment14d <= 13 ~ "Occasional", #1-13 days
    ment14d >= 14 ~ "Frequent" #14+ days
