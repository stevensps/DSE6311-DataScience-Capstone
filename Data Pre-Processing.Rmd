...
```{r}
rf_model <- rand_forest( # Random forest model with tuning for mtry and min_n
  mode = "classification",
  mtry = tune(),
  trees = 100,
  min_n = tune()
  ...
  set_engine("ranger",
             importance = "impurity",
             class.weights = c(Low = 1.00, High = 6.32)) # Using class weights to balance Low and High
```
...
```{r}
rf_model_final <- rand_forest( # Finalize RF model with best mtry and min_n values
  mode = "classification",
  mtry = best_rf$mtry,
  trees = 100,
  min_n = best_rf$min_n
...
```{r}
final_rf_wf <- workflow() %>% # Finalized model workflow
...
```{r}
final_rf_fit <- fit(final_rf_wf, data = traindataFinal) # Fit final RF model to training dataset
```
...
rf_results_prob <- bind_cols(testdataFinal, rf_probs) %>% # Combine predictions with test data
  mutate(pred_class_custom = if_else(.pred_High >= 0.25, "High", "Low")) %>% # Classify High if probability is greater than or equal to .25
  mutate(pred_class_custom = factor(pred_class_custom, levels = c("Low", "High")))
```
...
if (file.exists("final_xg_fit.rds")) {
  final_xg_fit <- readRDS("final_xg_fit.rds")
} else {
  set.seed(42)
  ...
  bestgrid_xgboost <- select_best(grid_tune, metric = "accuracy")
  
  final_xg_wf <- finalize_workflow(xgboost_wf, bestgrid_xgboost)
  
  final_xg_fit <- fit(final_xg_wf, data = traindataFinal)
  
  saveRDS(final_xg_fit, "final_xg_fit.rds")
}
```

#XG Boost Predictions - SS
```{r}
xgboost_preds <- predict(final_xg_fit, new_data = testdataFinal)

xgboost_results <- bind_cols(testdataFinal, xgboost_preds) %>%
  rename(pred_class = .pred_class)

accuracy(xgboost_results, truth = ment14d_cat, estimate = pred_class)
...
```{r}
xgboost_probs <- predict(final_xg_fit, new_data = testdataFinal, type = "prob") # Predict probabilities; apply 0.25 threshold
