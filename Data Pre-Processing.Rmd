...
```{r}
multi_model <- multinom_reg(mode = "classification") %>% # Set up multinomial logistic models
  set_engine("nnet")
```
...
```{r}
freqs <- table(train_clean$ment14d_cat)
...
rf_model <- rand_forest(
  ...
  set_engine("ranger",
             importance = "impurity",
             class.weights = c(None = 1.00, Occasional = 2.33, Frequent = 4.43))
```
