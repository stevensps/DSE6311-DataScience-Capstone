...
```{r}
multi_model <- multinom_reg(mode = "classification") %>% #Define multinomoal logistic regression model with nnet
  set_engine("nnet")
```
...
```{r}
multi_preds <- predict(multi_fit, new_data = testdataFinal) #Pedict class labels for test set
```
...
```{r}
test_results <- bind_cols(testdataFinal, multi_preds) %>% #Binf prediction to test set
...
```{r}
set.seed(123)
folds <- vfold_cv(trainimputeFinal, v = 5, strata = ment14d_cat) #Create 5-fold cv with stratification on target variable
...
multi_wf <- workflow() %>% #create formula workflow with model and target variable
  add_model(multi_model) %>%
...
```{r}
cv_results <- fit_resamples( #Perform 5-fold cv
  multi_wf,
  resamples = folds,
...
```{r}
collect_metrics(cv_results) #Mean accuracy across cv folds
