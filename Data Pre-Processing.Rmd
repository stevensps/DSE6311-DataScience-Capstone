...
```{r}
exists("max_depth", where = "package:dials")
```
...
rf_results <- tune_grid(
  ...
  resamples = folds,
  grid = rf_grid,
  metrics = metric_set(accuracy, recall, precision, kap),
...
```{r}
recall_compare <- data.frame(
  Dataset = c("Training CV Recall", "Test Set Recall"),
  Recall = c(0.927, 0.764)
)

ggplot(recall_compare, aes(x = Dataset, y = Recall, fill = Dataset)) +
  geom_col(width = 0.5) +
  geom_text(aes(label = scales::percent(Recall, accuracy = 0.1)), vjust = -0.5, size = 5) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  labs(
    title = "Training vs. Test Recall (High Class)",
    y = "Recall (%)",
    x = NULL
  ) +
  theme_minimal() +
